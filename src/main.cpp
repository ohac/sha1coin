...
#if defined(NDEBUG)
# error "Sha1coin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Sha1coin Signed Message:\n";
...
{
    // Sha1coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Sha1coin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Sha1coin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_Original(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sha1coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sha1coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Sha1coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x37e093ce45de215be68148927d85a7d80979ca96600a9284a61e83ed62fc4dad");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        if (fTestNet)
        {
            block.nTime    = 1394892853;
            block.nNonce   = 1837796;
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Sha1coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Sha1coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Sha1coinMiner : ProcessBlock, block not accepted");
    }
...
void static Sha1coinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Sha1coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Sha1coinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Sha1coinMiner, pwallet));
}
